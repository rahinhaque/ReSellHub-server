...
async function run() {
  try {
    ...
    // ── JWT: Generate token ───────────────────────────────────────────────────
    // PUBLIC — no verifyToken here, this is the login step
    app.post("/api/auth/token", async (req, res) => {
      try {
        const { email } = req.body;
        if (!email) return res.status(400).json({ error: "Email required" });

        // ✅ use the usersCollection already defined above (db.collection("user"))
    ...
    app.get("/api/products", async (req, res) => {
      ...
      const query = { status: "available", moderationStatus: "approved" };
    ...
    // PUBLIC — needed on product details page for seller info
    ...
    // PRIVATE — seller updates own product
    app.put(
    ...
    // PRIVATE — seller deletes own product
    app.delete(
      "/api/sellerProducts/:id",
      verifyToken,
      requireRole("seller", "admin"),
    ...
    // PRIVATE — buyer adds to wishlist
    app.post(
      "/api/wishlist",
      verifyToken,
      requireRole("buyer"),
    ...
    // PRIVATE — get wishlist by userId
    ...
    // PUBLIC — Stripe redirects here, no token in redirect context
    ...
    // PRIVATE — buyer views their orders
    app.get(
    ...
    // PRIVATE — seller views their orders
    app.get(
      "/api/orders/seller/:email",
      verifyToken,
      requireRole("seller", "admin"),
      async (req, res) => {
        try {
    ...
    // PRIVATE — seller updates order status
    app.patch(
    ...
    // PRIVATE — buyer payment history
    app.get(
      "/api/payments/buyer/:email",
      verifyToken,
      requireRole("buyer", "admin"),
      async (req, res) => {
        try {
    ...
    // PRIVATE — any logged-in user can report
    ...
    // PRIVATE — admin updates report status
    app.patch(
      "/api/reports/:id",
      verifyToken,
      requireRole("admin"),
    ...
    // PRIVATE — admin views all orders
    app.get(
      "/api/admin/orders",
      verifyToken,
      requireRole("admin"),
    ...
    // PRIVATE — admin resolves an order
    app.patch(
    ...
    // PRIVATE — admin payments overview
    app.get(
      "/api/admin/payments",
      verifyToken,
      requireRole("admin"),
